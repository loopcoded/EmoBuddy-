...
export default function GameToneOfVoice({ onComplete, onBack }: GameProps) {
  ...
  const scenarios = [
    {
      title: "Friendly Tone",
      text: "Hey, want to play with me? Come join!",
      question: "Is this tone friendly?",
      options: ["Yes, it's friendly", "No, it's rude", "It's neutral"],
      correct: 0,
    },
    {
      title: "Upset Tone",
      text: "Why did you take my pencil?! That's not fair!",
      question: "How does this sound?",
      options: ["Happy", "Upset or angry", "Confused"],
      correct: 1,
    },
    {
      title: "Polite Tone",
      text: "Could you please help me with this?",
      question: "Is this tone polite?",
      options: ["Yes, very polite", "No, it's mean", "It's confusing"],
      correct: 0,
    },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-b from-yellow-200 to-orange-200 flex items-center justify-center p-4">
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-yellow-600">Tone of Voice</h2>
          <motion.button
            onClick={onBack}
        ...
        >
          ...
          <h3 className="text-2xl font-bold text-yellow-600 mb-4">{scenarios[currentScenario].title}</h3>

          <motion.div
            initial={{ x: -10, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            className="bg-yellow-100 rounded-2xl p-6 mb-6"
          >
            <p className="text-lg text-gray-800 italic">{scenarios[currentScenario].text}</p>
          </motion.div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {scenarios[currentScenario].options.map((option, i) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white p-4 rounded-2xl font-bold hover:shadow-lg transition-all"
              >
                {option}
              </motion.button>
            ))}
